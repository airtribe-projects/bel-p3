...
const writableStream = fs.createWriteStream(path.join(__dirname, 'input.txt'), 'utf8');
...
while (bytesWritten < fileSizeInBytes) {
  ...
  const dataToWrite = remainingBytes > randomData.length ? randomData : randomData.slice(0, remainingBytes);
